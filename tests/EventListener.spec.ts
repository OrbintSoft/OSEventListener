...
describe('EventListener basic tests', function() {
	...
	it('dispatch', function() {
		const event = new EventListener('myEvent');
	...
	it('waits until first dispatch, not dispatched yet', function(done) {
		const event1 = new EventListener('myEvent');
		let called1 = 0;
	...
	it('it binds to another event', function() {
		let called = false;
		const event1 = new EventListener('A');
		const event2 = new EventListener('B');
		event1.bindToEvent(event2);
		event1.subscribe((sender, data) => {
	...
	it('it unbinds to another event', function() {
		let called = 0;
		const event1 = new EventListener('A');
		const event2 = new EventListener('B');
		event1.bindToEvent(event2);
